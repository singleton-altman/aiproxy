...
function DetailItem({ label, value, full = false }: { label: string; value: string; full?: boolean }) {
  ...
  return <View style={{ flexGrow: 1, flexBasis: full || !compactColumns ? '100%' : '46%', minWidth: 0, minHeight: 58, borderRadius: 14, backgroundColor: colors.mutedCard, paddingHorizontal: 11, paddingVertical: 9, gap: 4 }}><Text style={{ color: colors.subtext, fontSize: 11, fontWeight: '600' }}>{label}</Text><Text selectable numberOfLines={full ? 2 : 1} adjustsFontSizeToFit style={{ color: colors.text, fontSize: 11, lineHeight: 18, fontWeight: '700' }}>{value}</Text></View>;
}
