...
function QuotaWindowRow({ source, account }: { source: ApiRecord; account: ApiRecord }) {
  ...
  if (balance) {
    return <View style={{ gap: 6 }}>
      ...
      <View style={{ minHeight: 15, flexDirection: 'row', alignItems: 'center', gap: 5 }}>
        ...
        <Text numberOfLines={1} style={{ flex: 1, color: colors.subtext, fontSize: 11 }}>{resetLabel(source)}</Text>
        {limitText ? <Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11 }}>{limitText}</Text> : null}
      </View>
    </View>;
  }
  ...
  return <View style={{ gap: 5 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
      <Text numberOfLines={1} style={{ flex: 1, color: colors.text, fontSize: 11, fontWeight: '700' }}>{windowLabel(source)}</Text>
      <Text numberOfLines={1} style={{ color: unlimited ? colors.success : percent === undefined ? colors.subtext : progressColor, fontSize: 11, fontWeight: '800' }}>
    ...
    <View style={{ minHeight: 15, flexDirection: 'row', alignItems: 'center', gap: 5 }}>
      <Clock3 color={colors.subtext} size={12} />
      <Text numberOfLines={1} style={{ flex: 1, color: colors.subtext, fontSize: 11 }}>{resetLabel(source)}</Text>
      {remainingText ? <Text numberOfLines={1} style={{ maxWidth: '48%', color: colors.subtext, fontSize: 11 }}>{remainingText}</Text> : null}
    </View>
  </View>;
}
...
function QuotaGroupHeader({ group }: { group: QuotaGroup }) {
  ...
  return <View style={{ minHeight: 30, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
    ...
    <View style={{ paddingHorizontal: 8, paddingVertical: 4, borderRadius: 9, backgroundColor: colors.mutedCard }}><Text style={{ color: colors.subtext, fontSize: 11, fontWeight: '700' }}>{group.accounts.length} 个账号</Text></View>
  </View>;
}
  ...
  return <View style={{ minHeight: 154, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, padding: 12, gap: 10 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
      ...
      <View style={{ flex: 1, minWidth: 0, gap: 3 }}>
        ...
        <Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11 }}>{subtitle}</Text>
      </View>
      <View style={{ flexShrink: 0, flexDirection: 'row', alignItems: 'center', gap: 3 }}>
        <View style={{ paddingHorizontal: 7, paddingVertical: 4, borderRadius: 8, backgroundColor: statusColors.background }}>
          <Text style={{ color: statusColors.foreground, fontSize: 11, fontWeight: '800' }}>{status.text}</Text>
        </View>
    ...
    {windows.length ? <View style={{ gap: 9 }}>
      ...
      <Text style={{ color: colors.subtext, fontSize: 11 }}>刷新后等待上游返回额度窗口</Text>
    </View>}
    ...
    <View style={{ marginTop: 'auto', paddingTop: 8, borderTopWidth: 1, borderTopColor: colors.rowBorder, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
      <Text numberOfLines={1} style={{ flex: 1, minWidth: 90, color: colors.subtext, fontSize: 11 }}>{formatLastUsed(account.last_used_at ?? account.last_used)}</Text>
      <View style={{ maxWidth: '34%', paddingHorizontal: 7, paddingVertical: 3, borderRadius: 7, backgroundColor: colors.mutedCard }}><Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11, fontWeight: '700' }}>{plan}</Text></View>
      <View style={{ paddingHorizontal: 7, paddingVertical: 3, borderRadius: 7, backgroundColor: colors.primarySoft }}><Text style={{ color: colors.primary, fontSize: 11, fontWeight: '700' }}>优先级 {priority}</Text></View>
    </View>
  </View>;
}
...
export default function AdminQuotaScreen() {
  ...
  return <Page title="供应商配额" subtitle="按供应商分组，实时展示各上游账号的限额用量" icon={Coins} safeTop={false} refreshing={quota.isFetching || refresh.isPending}>
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
      ...
      <Text style={{ alignSelf: 'center', color: colors.subtext, fontSize: 11 }}>{groups.length} 个供应商 · {accountCount} 个账号</Text>
    </View>
