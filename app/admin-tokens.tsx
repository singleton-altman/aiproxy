...
export default function AdminTokensScreen() {
  ...
  return <Page title="管理令牌" subtitle={`${tokens.length} 项 · 程序访问管理接口的凭证`} icon={KeySquare} safeTop={false} refreshing={directory.isFetching || config.isFetching} onRefresh={() => { void directory.refetch(); void config.refetch(); }}>
    ...
    {filteredTokens.map((item, index) => {
      ...
      return <View key={tokenId(item) || `${tokenPrefix(item)}-${index}`} style={{ borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, padding: 12, gap: 9 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
          ...
          <View style={{ flex: 1, minWidth: 0, gap: 2 }}><Text numberOfLines={1} style={{ color: colors.text, fontSize: 14, fontWeight: '800' }}>{String(item.name || tokenId(item) || '令牌')}</Text><Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11, fontFamily: 'monospace' }}>{tokenPrefix(item)}</Text></View>
          <View style={{ minHeight: 24, paddingHorizontal: 8, borderRadius: 9, backgroundColor: statusBackground, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: statusColor, fontSize: 11, fontWeight: '800' }}>{statusText}</Text></View>
        </View>
        <View style={{ minHeight: 36, borderRadius: 11, backgroundColor: colors.mutedCard, paddingLeft: 10, flexDirection: 'row', alignItems: 'center', gap: 8 }}><ShieldCheck color={colors.primary} size={14} /><Pressable disabled={!scopes.length} onPress={() => Alert.alert('权限范围', scopes.map(scopeLabel).join('\n'))} style={{ flex: 1, minHeight: 36, flexDirection: 'row', alignItems: 'center', gap: 5 }}><Text numberOfLines={1} style={{ flex: 1, color: colors.primary, fontSize: 11, fontWeight: '700' }}>{scopeText}</Text>{scopes.length ? <Info color={colors.subtext} size={12} /> : null}</Pressable>{item.token ? <Pressable accessibilityLabel="复制完整令牌" onPress={() => void copyValue(String(item.token))} style={{ width: 34, height: 34, alignItems: 'center', justifyContent: 'center' }}>{copied === item.token ? <Check color={colors.success} size={14} /> : <Copy color={colors.subtext} size={14} />}</Pressable> : <View style={{ width: 6 }} />}</View>
        <View style={{ flexDirection: 'row', gap: 8 }}><View style={{ flex: 1 }}><Text style={{ color: colors.subtext, fontSize: 11 }}>最后使用</Text><Text numberOfLines={1} style={{ color: colors.text, fontSize: 11, marginTop: 2 }}>{item.last_used_at ? formatDate(item.last_used_at, true) : '从未使用'}</Text></View><View style={{ flex: 1 }}><Text style={{ color: colors.subtext, fontSize: 11 }}>有效期至</Text><Text numberOfLines={1} style={{ color: colors.text, fontSize: 11, marginTop: 2 }}>{item.expires_at ? formatDate(item.expires_at) : '永久有效'}</Text></View></View>
    ...
    <Panel>
      ...
      <Text style={{ color: colors.subtext, fontSize: 11, lineHeight: 16 }}>保护公开的 OAuth 回调路由。浏览器扩展会通过 X-Relay-Token 请求头发送相同的值。</Text>
      ...
      <View style={{ minHeight: 44, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, flexDirection: 'row', alignItems: 'center' }}>
        <TextInput value={relayDraft ?? ''} onChangeText={setRelayDraft} placeholder="留空表示不校验回调密钥" placeholderTextColor={colors.placeholder} secureTextEntry={!relayVisible} autoCapitalize="none" autoCorrect={false} style={{ flex: 1, minHeight: 42, paddingHorizontal: 12, color: colors.text, fontSize: 11, fontFamily: 'monospace' }} />
      ...
      <Text style={{ color: colors.subtext, fontSize: 11, lineHeight: 14 }}>服务器返回 ******** 时表示保持原值；清空并保存表示删除。本地回调可留空。</Text>
      {relayNeedsRestart ? <View style={{ borderRadius: 11, backgroundColor: colors.warningBg, padding: 9, flexDirection: 'row', gap: 7 }}><AlertTriangle color={colors.warning} size={14} /><Text style={{ flex: 1, color: colors.warning, fontSize: 11, lineHeight: 14 }}>配置已保存到磁盘，需要重启服务后回调路由才会使用新密钥。</Text></View> : null}
        ...
        <View style={{ height: sheetHeight, width: '100%', maxWidth: 720, alignSelf: 'center', borderTopLeftRadius: 22, borderTopRightRadius: 22, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.page, padding: 16, gap: 10 }}>
          ...
          <ScrollView bounces={false} alwaysBounceVertical={false} overScrollMode="never" keyboardShouldPersistTaps="handled" style={{ flex: 1 }} contentContainerStyle={{ gap: 12, paddingBottom: 8 }}>
            ...
            <View style={{ gap: 7 }}><Text style={{ color: colors.text, fontSize: 11, fontWeight: '700' }}>权限预设</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7 }}>{presets.map((preset) => {
              ...
              return <Pressable key={preset.id} onPress={() => setSelectedScopes(preset.scopes)} style={{ minHeight: 36, paddingHorizontal: 10, borderRadius: 11, borderWidth: 1, borderColor: active ? colors.primary : colors.border, backgroundColor: active ? colors.primarySoft : colors.card, flexDirection: 'row', alignItems: 'center', gap: 5 }}><PresetIcon color={active ? colors.primary : colors.subtext} size={14} /><Text style={{ color: active ? colors.primary : colors.text, fontSize: 11, fontWeight: '700' }}>{preset.label}</Text></Pressable>;
            })}</View></View>
            <View style={{ borderRadius: 14, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' }}>{groups.map((group) => {
              ...
              return <View key={group} style={{ borderBottomWidth: group === groups[groups.length - 1] ? 0 : 1, borderBottomColor: colors.rowBorder }}><View style={{ minHeight: 34, paddingHorizontal: 10, backgroundColor: colors.mutedCard, flexDirection: 'row', alignItems: 'center' }}><Text style={{ flex: 1, color: colors.text, fontSize: 11, fontWeight: '800' }}>{groupLabel(group)}</Text>{selectedCount ? <Text style={{ color: colors.subtext, fontSize: 11 }}>{selectedCount}/{definitions.length}</Text> : null}</View>{definitions.map((definition) => {
            ...
            <View style={{ borderRadius: 12, backgroundColor: colors.mutedCard, padding: 11, flexDirection: 'row', gap: 8 }}><Info color={colors.text} size={15} /><Text style={{ flex: 1, color: colors.text, fontSize: 11, lineHeight: 15 }}>拿到令牌的人无需登录即可执行所选操作，请妥善保存。</Text></View>
          </ScrollView>
          {create.error ? <Text style={{ color: colors.danger, fontSize: 11 }}>{create.error.message}</Text> : null}
    ...
    <Modal visible={Boolean(createdToken)} transparent animationType="fade" onRequestClose={() => setCreatedToken('')}>
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'center', padding: 20, backgroundColor: 'rgba(0,0,0,0.45)' }}>
        <View style={{ width: '100%', maxWidth: 520, alignSelf: 'center', borderRadius: 20, backgroundColor: colors.page, padding: 18, gap: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}><View style={{ flex: 1 }}><Text style={{ color: colors.text, fontSize: 17, fontWeight: '800' }}>令牌已创建</Text><Text style={{ color: colors.subtext, fontSize: 11, marginTop: 3 }}>完整令牌仅显示这一次</Text></View><Pressable accessibilityLabel="关闭" onPress={() => setCreatedToken('')} style={{ width: 34, height: 34, alignItems: 'center', justifyContent: 'center' }}><X color={colors.subtext} size={17} /></Pressable></View>
          ...
          <View style={{ borderRadius: 12, backgroundColor: colors.warningBg, padding: 10, flexDirection: 'row', gap: 7 }}><AlertTriangle color={colors.warning} size={14} /><Text style={{ flex: 1, color: colors.warning, fontSize: 11, lineHeight: 14 }}>关闭后无法再次查看，请立即复制并安全保存。</Text></View>
