...
export default function AdminUsersScreen() {
  ...
  return <Page title="用户" subtitle="管理用户角色、状态与套餐分配" icon={UsersRound} safeTop={false} contentMaxWidth={1180} scrollable={false} refreshing={users.isFetching} onRefresh={() => users.refetch()}>
    ...
    <FlatList
      ...
      contentContainerStyle={{ gap: 8, paddingBottom: 96, flexGrow: items.length ? 0 : 1 }}
    ...
    <Modal visible={Boolean(selected)} transparent animationType="slide" onRequestClose={() => setSelected(undefined)}>
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.45)' }}>
        {selected ? <View style={{ maxHeight: '92%', borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: colors.page, padding: 16, gap: 12 }}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" style={{ flexGrow: 0 }} contentContainerStyle={{ gap: 11, paddingBottom: 4 }}>
