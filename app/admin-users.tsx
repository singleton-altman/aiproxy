    ...
    <FlatList
      ...
      renderItem={({ item }) => {
        ...
        return <Pressable onPress={() => openUser(item)} style={({ pressed }) => ({ minHeight: 72, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 10, opacity: pressed ? 0.68 : 1 })}>
          <View style={{ width: 36, height: 36, borderRadius: 12, backgroundColor: colors.mutedCard, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: colors.subtext, fontSize: 11, fontWeight: '800' }}>{nickname(item).slice(0, 1).toUpperCase()}</Text></View>
          <View style={{ flex: 1, minWidth: 0, gap: 5 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}><Text numberOfLines={1} style={{ flex: 1, color: colors.text, fontSize: 11, fontWeight: '800' }}>{String(item.email ?? '未知邮箱')}</Text><View style={{ paddingHorizontal: 7, paddingVertical: 3, borderRadius: 7, backgroundColor: role === 'user' ? colors.mutedCard : colors.primarySoft }}><Text style={{ color: role === 'user' ? colors.subtext : colors.primary, fontSize: 11, fontWeight: '800' }}>{roleLabel(role)}</Text></View><View style={{ paddingHorizontal: 7, paddingVertical: 3, borderRadius: 7, backgroundColor: disabled ? colors.dangerBg : colors.successBg }}><Text style={{ color: disabled ? colors.danger : colors.success, fontSize: 11, fontWeight: '800' }}>{disabled ? '禁用' : '启用'}</Text></View></View>
            <Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11 }}>{nickname(item)} · 注册 {formatDate(item.created_at)} · 余额 {formatBalance(item.balance ?? 0)}</Text>
          </View>
        </Pressable>;
      }}
    />
    ...
    <Modal visible={Boolean(selected)} transparent statusBarTranslucent navigationBarTranslucent animationType="slide" onRequestClose={() => setSelected(undefined)}>
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: colors.sheetBackdrop }}>
        {selected ? <View style={{ maxHeight: '92%', borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: colors.page, padding: 16, gap: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}><View style={{ flex: 1, minWidth: 0 }}><Text style={{ color: colors.text, fontSize: 16, fontWeight: '800' }}>编辑用户</Text><Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 11, marginTop: 3 }}>更新 {String(selected.email ?? userId(selected))} 的账户信息</Text></View><Pressable accessibilityLabel="关闭" onPress={() => setSelected(undefined)} style={{ width: 34, height: 34, alignItems: 'center', justifyContent: 'center' }}><X color={colors.subtext} size={17} /></Pressable></View>
          <ScrollView bounces={false} alwaysBounceVertical={false} overScrollMode="never" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="handled" style={{ flexGrow: 0 }} contentContainerStyle={{ gap: 11, paddingBottom: 4 }}>
            ...
            {draft.password && !passwordValid ? <Text style={{ color: colors.danger, fontSize: 11 }}>密码至少 8 个字符</Text> : null}
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}><Pressable onPress={() => setSelected(undefined)} style={{ flex: 1, minHeight: 44, borderRadius: 12, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: colors.text, fontSize: 11, fontWeight: '700' }}>取消</Text></Pressable><Pressable disabled={!canSave} onPress={() => saveMutation.mutate()} style={{ flex: 1, minHeight: 44, borderRadius: 12, backgroundColor: canSave ? colors.primary : colors.disabled, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }}>{saveMutation.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Save color="#fff" size={14} />}<Text style={{ color: '#fff', fontSize: 11, fontWeight: '800' }}>保存</Text></Pressable></View>
        </View> : null}
      </FullScreenSafeArea>
    </Modal>
  </Page>;
