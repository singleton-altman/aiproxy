...
import { getApiEndpoints, getApiModules } from '@/src/services/endpoints';
import type { ApiEndpointDefinition } from '@/src/types/api';
...
function methodColor(method: string, colors: ReturnType<typeof useAppTheme>) {
  ...
  if (method === 'PUT' || method === 'PATCH') return colors.warning;
...
export default function ModuleEndpointsScreen() {
  ...
  const module = useMemo(() => getApiModules().find((item) => item.key === moduleKey), [moduleKey]);
  const [search, setSearch] = useState('');
  const endpoints = useMemo(() => {
    const keyword = search.trim().toLowerCase();
    return getApiEndpoints(moduleKey).filter((endpoint) => !keyword || `${endpoint.path} ${endpoint.id} ${endpoint.notes}`.toLowerCase().includes(keyword));
  }, [moduleKey, search]);

  if (!module) return <Page title="模块不存在" icon={Braces}><EmptyState message="接口清单中没有找到该模块" icon={Braces} /></Page>;

  return <Page title={module.label} subtitle={`${module.endpointCount} 个端点 · ${module.methodCount} 个方法`} icon={Braces} scrollable={false} safeTop={false}>
    <SearchField value={search} onChangeText={setSearch} placeholder="搜索路径或说明" />
...
function EndpointRow({ endpoint, onPress }: { endpoint: ApiEndpointDefinition; onPress: () => void }) {
