...
export default function PlansScreen() {
  ...
  return <Page title="套餐计划" subtitle="可用套餐与额度规则" icon={Package} safeTop={false} refreshing={plans.isFetching} onRefresh={() => plans.refetch()}>
    ...
    {(plans.data ?? []).map((plan, index) => <Panel key={String(plan.id ?? index)}>
      ...
      {plan.limits && typeof plan.limits === 'object' ? <View style={{ gap: 6 }}>{Object.entries(plan.limits).map(([key, value]) => <View key={key} style={{ minHeight: 32, flexDirection: 'row', alignItems: 'center', gap: 10, borderTopWidth: 1, borderTopColor: colors.rowBorder }}><Text style={{ flex: 1, color: colors.subtext, fontSize: 11 }}>{key}</Text><Text style={{ color: colors.text, fontSize: 11, fontWeight: '700' }}>{typeof value === 'object' ? JSON.stringify(value) : String(value)}</Text></View>)}</View> : null}
    </Panel>)}
