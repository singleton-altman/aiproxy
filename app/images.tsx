...
export default function ImagesScreen() {
  ...
  return <Page title="图像生成" subtitle="POST /v1/images/generations" icon={ImagePlus} safeTop={false} refreshing={mutation.isPending || gatewayModels.isFetching} onRefresh={refreshModels}>
    ...
    {mutation.error ? <>
      ...
      {/上游账号均处于冷却/.test(mutation.error.message) && isAdmin() ? <Pressable onPress={() => router.push('/admin-accounts' as never)} style={{ minHeight: 42, borderRadius: 12, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: colors.primary, fontSize: 12, fontWeight: '800' }}>检查上游账号</Text></Pressable> : null}
    </> : null}
