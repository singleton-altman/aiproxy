            ...
            return <View style={{ flexDirection: 'row', justifyContent: mine ? 'flex-end' : 'flex-start', gap: 7 }}>
              ...
              <View style={{ maxWidth: '84%', gap: 4 }}>
                ...
                {!mine && (item.content || usage || item.error) ? <View style={{ minHeight: 24, flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                  {usage ? <Text numberOfLines={1} style={{ flex: 1, color: colors.subtext, fontSize: 11 }}>{usage}</Text> : <View style={{ flex: 1 }} />}
    ...
    <Modal visible={settingsOpen} transparent statusBarTranslucent navigationBarTranslucent animationType="slide" onRequestClose={() => setSettingsOpen(false)}>
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: colors.sheetBackdrop }}>
        <View style={{ height: settingsSheetHeight, width: '100%', maxWidth: 720, alignSelf: 'center', borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: colors.page, padding: 16, gap: 10 }}>
          ...
          <ScrollView bounces={false} alwaysBounceVertical={false} overScrollMode="never" automaticallyAdjustKeyboardInsets keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="handled" style={{ flex: 1 }} contentContainerStyle={{ gap: 10, paddingBottom: 4 }}>
            ...
            <View style={{ minHeight: 48, borderTopWidth: 1, borderTopColor: colors.rowBorder, flexDirection: 'row', alignItems: 'center', gap: 10 }}><View style={{ flex: 1 }}><Text style={{ color: colors.text, fontSize: 11, fontWeight: '700' }}>流式响应</Text><Text style={{ color: colors.subtext, fontSize: 11 }}>关闭后使用普通 JSON 响应</Text></View><AppSwitch accessibilityLabel="流式响应" value={streamEnabled} onValueChange={setStreamEnabled} /></View>
          </ScrollView>
