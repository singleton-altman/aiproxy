...
import { Activity, BarChart3, Boxes, CloudCog, Coins, FileUp, KeySquare, LayoutGrid, Network, Package, ServerCog, Settings2, ShieldAlert, TicketPercent, UsersRound, Waypoints } from 'lucide-react-native';
...
export default function AdminScreen() {
  ...
  return <Page title="管理" subtitle={admin ? '管理端功能' : '当前账号可能没有管理员权限'} icon={LayoutGrid} refreshing={refreshing || stats.isFetching} onRefresh={refresh}>
    {!admin ? <Panel>
      ...
      <Text style={{ color: colors.subtext, fontSize: 11, lineHeight: 18 }}>未检测到管理员角色。若你确认自己是管理员，下方功能仍可尝试访问；无权限的请求会返回错误。</Text>
    </Panel> : null}
