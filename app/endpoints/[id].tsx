...
import {
  callApiEndpoint,
  getApiEndpoint,
  isDangerousApiRequest,
  type EndpointResult,
} from '@/src/services/endpoints';
import type { ApiRecord, HttpMethod } from '@/src/types/api';
...
function formatBinary(result: EndpointResult) {
  return {
    状态: result.status,
...
function safeFilename(value: string | undefined) {
  return (value?.trim() || 'ai-proxy-download.bin').replace(/[<>:"/\\|?*\u0000-\u001f]/g, '_');
}
...
export default function EndpointRunnerScreen() {
  ...
  const endpoint = useMemo(() => getApiEndpoint(id), [id]);
  const [method, setMethod] = useState<HttpMethod>('GET');
  const [pathValues, setPathValues] = useState<Record<string, string>>({});
  const [queryMode, setQueryMode] = useState<RootMode>('object');
  const [queryObject, setQueryObject] = useState<ApiRecord>({});
  const [queryArray, setQueryArray] = useState<ApiRecord[]>([]);
  const [bodyMode, setBodyMode] = useState<RootMode>('object');
  const [bodyObject, setBodyObject] = useState<ApiRecord>({});
  ...
  const fileCapable = Boolean(endpoint && method !== 'GET' && /\/(?:import|upload)(?:[/?]|$)/i.test(endpoint.path));
        ...
        const body = method === 'GET'
          ...
          : fileCapable && selectedFile
            ...
            : sendBody
              ? formValue
              : undefined;
        return await callApiEndpoint({ endpoint, method, pathValues, query, body, signal: controller.signal });
      } finally {
  ...
  function run() {
    ...
    if (missing.length) {
      setInputError(`请填写路径参数 ${missing.join(', ')}`);
      return;
    }
    const execute = () => {
      mutation.reset();
    ...
    if (dangerous) {
      Alert.alert('确认执行高风险接口', `${method} ${selectedEndpoint.path}\n\n该请求可能修改或删除服务数据、账号或系统状态。`, [{ text: '取消', style: 'cancel' }, { text: '确认执行', style: 'destructive', onPress: execute }]);
    } else execute();
  }
