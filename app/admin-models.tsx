...
function MobileModelCard({ item, busy, onToggle, onEdit, onDelete }: { item: ApiRecord; busy: boolean; onToggle: (enabled: boolean) => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <View style={{ marginHorizontal: 1, borderWidth: 1, borderTopWidth: 0, borderColor: colors.border, backgroundColor: colors.card, padding: 12, gap: 10 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
      ...
      <Pressable accessibilityLabel="编辑模型" onPress={onEdit} style={{ width: 34, height: 34, borderRadius: 8, backgroundColor: colors.mutedCard, alignItems: 'center', justifyContent: 'center' }}><SlidersHorizontal color={colors.primary} size={15} /></Pressable>
      <Pressable accessibilityLabel="删除模型" onPress={onDelete} style={{ width: 34, height: 34, borderRadius: 8, backgroundColor: colors.dangerBg, alignItems: 'center', justifyContent: 'center' }}><Trash2 color={colors.danger} size={15} /></Pressable>
    </View>
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 9, padding: 10, borderRadius: 8, backgroundColor: colors.mutedCard }}>
        ...
        if (entry.kind === 'provider') {
          ...
          return <View>
            <Pressable onPress={() => toggleGroup(entry.group.provider)} style={({ pressed }) => ({ marginTop: 10, minHeight: 56, borderWidth: 1, borderColor: colors.border, borderTopLeftRadius: 8, borderTopRightRadius: 8, borderBottomLeftRadius: open ? 0 : 8, borderBottomRightRadius: open ? 0 : 8, backgroundColor: colors.card, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 9, opacity: pressed ? 0.7 : 1 })}>
              ...
              <View style={{ width: 30, height: 30, borderRadius: 8, backgroundColor: colors.mutedCard, alignItems: 'center', justifyContent: 'center' }}><Boxes color={colors.text} size={15} /></View>
              <Text numberOfLines={1} style={{ flex: 1, color: colors.text, fontSize: 13, fontWeight: '800' }}>{entry.group.provider}</Text>
              <View style={{ paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: colors.mutedCard }}><Text style={{ color: colors.text, fontSize: 9, fontWeight: '700' }}>{entry.group.items.length} 个模型</Text></View>
              <Pressable disabled={groupBusy} onPress={(event) => { event.stopPropagation(); void maintenance('probe', { provider: entry.group.provider }); }} style={{ minHeight: 34, paddingHorizontal: 9, borderRadius: 8, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 5 }}>{groupBusy ? <ActivityIndicator color={colors.primary} size="small" /> : <Radar color={colors.primary} size={13} />}<Text style={{ color: colors.primary, fontSize: 10, fontWeight: '800' }}>探测</Text></Pressable>
            </Pressable>
    ...
    <Modal visible={Boolean(formMode)} transparent animationType="slide" onRequestClose={() => setFormMode('')}>
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.45)' }}><View style={{ maxHeight: '90%', borderTopLeftRadius: 16, borderTopRightRadius: 16, backgroundColor: colors.page, padding: 16, gap: 12 }}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" style={{ flexGrow: 0 }} contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, paddingBottom: 8 }}>
          ...
          {([['启用模型', 'enabled'], ['注册表隐藏', 'registry_hidden'], ['用户隐藏', 'user_hidden']] as const).map(([label, key]) => <View key={key} style={{ flexGrow: 1, flexBasis: '30%', minHeight: 44, borderRadius: 8, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 10, flexDirection: 'row', alignItems: 'center', gap: 8 }}><Text style={{ flex: 1, color: colors.text, fontSize: 11, fontWeight: '700' }}>{label}</Text><Switch value={draft[key]} onValueChange={(value) => setDraft((current) => ({ ...current, [key]: value }))} trackColor={{ false: colors.disabled, true: colors.primary }} /></View>)}
        </ScrollView>
        <Pressable disabled={formMutation.isPending || !draft.id.trim()} onPress={() => formMutation.mutate()} style={{ minHeight: 48, borderRadius: 8, backgroundColor: draft.id.trim() ? colors.primary : colors.disabled, alignItems: 'center', justifyContent: 'center' }}>{formMutation.isPending ? <ActivityIndicator color="#fff" /> : <Text style={{ color: '#fff', fontWeight: '800' }}>保存模型</Text>}</Pressable>
