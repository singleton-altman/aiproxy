      ...
      <FullScreenSafeArea style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: colors.sheetBackdrop }}>
        <View style={{ height: sheetHeight, width: '100%', maxWidth: 720, alignSelf: 'center', borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: colors.page, padding: 16, gap: 10 }}>
          ...
          <FlatList
            ...
            ListHeaderComponent={value.trim() && !hasListedCurrent ? <Pressable onPress={() => selectSecret(value)} style={{ minHeight: 54, borderRadius: 12, borderWidth: 1, borderColor: colors.primary, backgroundColor: colors.primarySoft, paddingHorizontal: 12, justifyContent: 'center', gap: 3 }}><Text style={{ color: colors.text, fontSize: 11, fontWeight: '700' }}>本机已保存 Key</Text><Text numberOfLines={1} style={{ color: colors.primary, fontSize: 11, fontFamily: 'monospace' }}>{maskedPrefix(value)}</Text></Pressable> : null}
